...
namespace POS.Tests.Category
{
    ...
    public class CategoryApplicationTest
    {
        ...
        public async Task RegisterCategory_WhenSendingNullValuesOrEmpty_ValidationErrors()
        {
            ...
            var corrent = result.Message;

            //Assert
            Assert.AreEqual(expected, corrent);
        }

        [TestMethod]
        public async Task RegisterCategory_WhenSendingCorrectValues_RegisteredSuccessful()
        {
            using var scope = _scopeFactory!.CreateScope();
            var context = scope?.ServiceProvider.GetService<ICategoryApplication>();

            //Arrange
            var name = "Nuevo categoria";
